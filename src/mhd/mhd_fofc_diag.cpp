...
void MHD::FOFCDiag(Driver *pdriver, int stage) {
  ...
  KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
    ...
    if (fofc_flag || fofc_excision) {
      ...
      // compute new 1st-order LLF flux at i-face
      {
        ...
        if (eos.is_ideal) {
          if constexpr (true) fofc_diag1_(m,k,j,i) += flux.e-flx1(m,IEN,k,j,i);
      ...
      if (multi_d) {
        ...
        if (eos.is_ideal) {
          if constexpr (true) fofc_diag2_(m,k,j,i) += flux.e-flx2(m,IEN,k,j,i);
      ...
      if (three_d) {
        ...
        if (eos.is_ideal) {
          if constexpr (true) fofc_diag3_(m,k,j,i) += flux.e-flx3(m,IEN,k,j,i);
  ...
  KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
    ...
    if (fofc_flag || fofc_excision) {
      ...
      // compute new 1st-order LLF flux at (i+1)-face
      {
        ...
        if (eos.is_ideal) {
          if constexpr (true) fofc_diag1_(m,k,j,i+1) += flux.e-flx1(m,IEN,k,j,i+1);
      ...
      if (multi_d) {
        ...
        if (eos.is_ideal) {
          if constexpr (true) fofc_diag2_(m,k,j+1,i) += flux.e-flx2(m,IEN,k,j+1,i);
      ...
      if (three_d) {
        ...
        if (eos.is_ideal) {
          if constexpr (true) fofc_diag3_(m,k+1,j,i) += flux.e-flx3(m,IEN,k+1,j,i);
