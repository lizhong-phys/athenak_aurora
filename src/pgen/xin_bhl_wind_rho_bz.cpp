...
//========================================================================================
//! \file xin_bhl_wind_rho_bz.cpp
//! \brief Problem generator: Bondi-Hoyle-Lyttleton wind with a transverse
//!        density and velocity gradient onto a Kerr BH.  The medium streams
//!        along +x with uniform temperature and y-dependent upstream state
//!          v_x(y) = v_ref + delta_v*tanh(y/v_grad_r),
//!          rho(y) = rho_ref + delta_rho*tanh(y/rho_grad_r),
//!        where v_ref=(v_neg_y+v_pos_y)/2,
//!        delta_v=(v_pos_y-v_neg_y)/2,
//!        rho_ref=(rho_neg_y+rho_pos_y)/2, and
//!        delta_rho=(rho_pos_y-rho_neg_y)/2.
//!        Injection is on the -x face (inner_x1); the other 5 faces are outflow.
//!        The asymptotic B-field is spatially uniform along +z (perpendicular to
//!        the reference wind and parallel to the spin axis +z), i.e. theta_B = 0 deg
//!        in the convention of Kim & Most 2025 (Phys. Rev. D 111, 083025), Eq. 21.
//!
//! Inputs (all physical, units of c=1 and the code density unit):
//!    t0          : pressure-per-density p/rho
//!    v_neg_y     : physical wind speed approached as y -> -infinity
//!    v_pos_y     : physical wind speed approached as y -> +infinity
//!    v_grad_r    : transverse scale of the tanh velocity gradient
//!    rho_neg_y   : rest-mass density approached as y -> -infinity
//!    rho_pos_y   : rest-mass density approached as y -> +infinity
//!    rho_grad_r  : transverse scale of the tanh density gradient
//!    beta_target : reference total thermal-pressure beta
//!                  = 2*(p_gas+p_rad)/b^2_comoving, evaluated at y=0
//!
//! Initial state (Cartesian Kerr-Schild):
//!    rho      = rho(y)                                     outside excision
//!    p        = rho(y) * t0                                (uniform T = t0)
//!    uu^i     = (gamma(y) * v_x(y), 0, 0)                  outside excision
//!               (the relativistic normal-frame primitive velocity)
//!    uu^i     = (0, 0, 0)                                  inside excision
//!    A_y      = b0 * x1,  A_x = A_z = 0                    (-> uniform B = b0 z_hat)
//!
//! B-field amplitude b0 (coordinate-frame B^z) is set from the TOTAL thermal
//! pressure and reference state at y=0:
//!    p_gas,ref = rho_ref*t0,
//!    p_rad = arad*t0^4/3 when radiation is enabled (zero otherwise),
//!    b0 = sqrt(2*(p_gas,ref+p_rad)/(beta_target*(1-v_ref^2))).
//! The (1-v_ref^2) factor accounts for b^2_comoving = B^2*(1-v_ref^2) for
//! B perpendicular to the reference wind.  Since B is kept uniform while v_x
//! and both rho and v_x vary with y, beta_target is realized exactly at the
//! reference slice y=0; the local comoving beta varies across the gradients.
//!
//! Boundaries:
//!    inner_x1 (-x face): INJECTION.  Dirichlet on hydro
//!                       (rho(y), p=rho(y)*t0,
//!                       uu^x=gamma(y)*v_x(y)), face-centered B pinned to
//!                       (0, 0, b0), and thermal radiation at t0 boosted with
//!                       the same local wind velocity as the IC.
//!    other 5 faces:     zero-gradient outflow for hydro/B (primitives copied
//!                       from innermost active cell; face B copied).
//!
//! CORRECTED RADIATION BC (vs xin_bhl_xy_final.cpp): on the 5 non-injection
//! faces, INCOMING radiation directions are no longer set to zero (which acted
//! as a one-sided radiative sink, cooling the edge gas).  They are instead set
//! to the ambient wind-boosted thermal-equilibrium intensity at t0 (same analytic
//! formula as the IC / inner_x1 injection), via WindEquilibIntensity().  OUTGOING
//! directions keep the zero-gradient copy, so radiation still escapes freely.
//! Net effect: edge gas is held in radiative equilibrium at t0 while outflow of
//! both gas and radiation is preserved.  Hydro and B BCs are unchanged.
//!
//! Inside the horizon: primitives replaced by excision floors (dexcise, pexcise).
//!
//! Derived from xin_bhl_wind_rho.cpp, with the field rotated from +y to +z.
...
// Parameters for the BHL-wind setup (wind along +x, B along +z)
struct bhl_pgen {
  ...
  // Upstream-wind state: uniform T with transverse density and velocity gradients.
  Real t0;              // p/rho ("temperature"); set directly
  Real v_neg_y;         // physical v_x approached as y -> -infinity
  Real v_pos_y;         // physical v_x approached as y -> +infinity
  Real v_ref;           // reference physical speed, (v_neg_y + v_pos_y)/2
  Real v_grad_r;        // transverse tanh-gradient scale
  Real rho_neg_y;       // density approached as y -> -infinity
  Real rho_pos_y;       // density approached as y -> +infinity
  Real rho_ref;         // reference density, (rho_neg_y + rho_pos_y)/2
  Real rho_grad_r;      // transverse tanh-gradient scale

  // Uniform vertical magnetic field B = b0 * z_hat (theta_B=0 deg).
  // Coordinate-frame B^z from A_y = b0*x1 (curl gives B^z = b0).
  // b0 is set from total thermal beta at v_ref and remains spatially uniform.
  Real b0;              // coordinate-frame vertical field amplitude
};

KOKKOS_INLINE_FUNCTION
Real WindVelocity(const bhl_pgen &pgen, Real y);

KOKKOS_INLINE_FUNCTION
Real WindPrimitiveVelocity(const bhl_pgen &pgen, Real y);

KOKKOS_INLINE_FUNCTION
Real WindDensity(const bhl_pgen &pgen, Real y);
...
//! \brief Density-and-velocity-gradient BHL wind onto a Kerr BH along +x with
//!        uniform B along +z.
//! Compile with
//! '-D PROBLEM=xin_bhl_wind_rho_bz'.
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (!pmbp->pcoord->is_general_relativistic &&
    ...
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
              << "Gradient BHL problem can only be run when GR defined in <coord> block"
              << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  // Five MHD spheres use all 20 available user-history reduction slots.
  ...
  // Inputs are physical velocities; AthenaK stores gamma*v as its normal-frame
  // primitive velocity.  Side values are named by position, not magnitude, so
  // either profile may increase or decrease with y without special cases.
  bhl.t0      = pin->GetReal("problem", "t0");
  bhl.v_neg_y = pin->GetReal("problem", "v_neg_y");
  bhl.v_pos_y = pin->GetReal("problem", "v_pos_y");
  bhl.v_grad_r = pin->GetReal("problem", "v_grad_r");
  bhl.rho_neg_y = pin->GetReal("problem", "rho_neg_y");
  bhl.rho_pos_y = pin->GetReal("problem", "rho_pos_y");
  bhl.rho_grad_r = pin->GetReal("problem", "rho_grad_r");
  if (bhl.v_neg_y <= 0.0 || bhl.v_neg_y >= 1.0) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
              << "v_neg_y must be in (0, 1); got " << bhl.v_neg_y << std::endl;
    exit(EXIT_FAILURE);
  }
  if (bhl.v_pos_y <= 0.0 || bhl.v_pos_y >= 1.0) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
              << "v_pos_y must be in (0, 1); got " << bhl.v_pos_y << std::endl;
    exit(EXIT_FAILURE);
  }
  if (bhl.v_grad_r <= 0.0) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
              << "v_grad_r must be positive; got " << bhl.v_grad_r << std::endl;
    exit(EXIT_FAILURE);
  }
  if (bhl.rho_neg_y <= 0.0) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl << "rho_neg_y must be positive; got " << bhl.rho_neg_y
              << std::endl;
    exit(EXIT_FAILURE);
  }
  if (bhl.rho_pos_y <= 0.0) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl << "rho_pos_y must be positive; got " << bhl.rho_pos_y
              << std::endl;
    exit(EXIT_FAILURE);
  }
  if (bhl.rho_grad_r <= 0.0) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl << "rho_grad_r must be positive; got "
              << bhl.rho_grad_r << std::endl;
    exit(EXIT_FAILURE);
  }
  bhl.v_ref = 0.5 * (bhl.v_neg_y + bhl.v_pos_y);
  bhl.rho_ref = 0.5 * (bhl.rho_neg_y + bhl.rho_pos_y);

  // excision parameters
  bhl.dexcise = coord.dexcise;
  bhl.pexcise = coord.pexcise;

  // Uniform vertical field amplitude b0 (coordinate-frame B^z) from the
  // reference total thermal-pressure beta.  This MUST be computed here, BEFORE
  // the restart return: WindBCs
  // reads bhl.b0 (as bz_inj) to inject the field and runs on restart too, so
  ...
  if (pmbp->pmhd != nullptr) {
    ...
    if (beta_target <= 0.0) {
      std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
                << std::endl << "beta_target must be positive; got " << beta_target
                << std::endl;
      exit(EXIT_FAILURE);
    }
    // beta_target = (p_gas+p_rad)/(b^2_comoving/2), evaluated at v_ref.
    // arad*T^4 is radiation ENERGY density; isotropic radiation pressure is
    // p_rad=arad*T^4/3.  It vanishes naturally when radiation is disabled.
    Real p_gas = bhl.rho_ref * bhl.t0;
    Real p_rad = (is_radiation_enabled) ? bhl.arad * SQR(SQR(bhl.t0)) / 3.0 : 0.0;
    Real p_thermal_tot = p_gas + p_rad;
    Real one_m_vref2 = 1.0 - SQR(bhl.v_ref);
    bhl.b0 = sqrt(2.0 * p_thermal_tot / (beta_target * one_m_vref2));
  }
  ...
  //   rho and uu^x follow transverse profiles; p=rho*t0 outside excision
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    Real uu1, uu2, uu3;
    if (r_excise > 1.0) {
      // Isothermal wind state with local transverse density and velocity profiles.
      rho  = WindDensity(trs, x2v);
      pgas = rho * trs.t0;
      if (is_radiation_enabled) urad = trs.arad * SQR(SQR(trs.t0));
      uu1 = WindPrimitiveVelocity(trs, x2v);  uu2 = 0.0;  uu3 = 0.0;
    } else {
  ...
  if (pmbp->pmhd != nullptr) {
    // Uniform vertical B = b0 * z_hat (coordinate-frame B^z).  bhl.b0 was
    // already computed above (before the restart return) from total thermal
    // pressure and v_ref.  It does not depend on the local v_x(y), so B stays uniform.
...
//----------------------------------------------------------------------------------------
// Physical wind velocity and corresponding AthenaK normal-frame primitive velocity.

KOKKOS_INLINE_FUNCTION
Real WindVelocity(const bhl_pgen &pgen, Real y) {
  Real delta_v = 0.5 * (pgen.v_pos_y - pgen.v_neg_y);
  return pgen.v_ref + delta_v * tanh(y / pgen.v_grad_r);
}

KOKKOS_INLINE_FUNCTION
Real WindPrimitiveVelocity(const bhl_pgen &pgen, Real y) {
  Real vloc = WindVelocity(pgen, y);
  return vloc / sqrt(1.0 - SQR(vloc));
}

KOKKOS_INLINE_FUNCTION
Real WindDensity(const bhl_pgen &pgen, Real y) {
  Real delta_rho = 0.5 * (pgen.rho_pos_y - pgen.rho_neg_y);
  return pgen.rho_ref + delta_rho * tanh(y / pgen.rho_grad_r);
}
...
//----------------------------------------------------------------------------------------
// Vector potential for a uniform B-field along +z in Cartesian Kerr-Schild:
//   A_y = b0 * x1,  A_x = A_z = 0  =>  B = curl A = (0, 0, b0).
// Same prescription as Kim & Most 2025 (their Eq. 21 with theta_B = 0 deg):
...
//  moving with local normal-frame velocity (uu_loc, 0, 0), evaluated for angle n at
...
//! \fn WindBCs
//  \brief Gradient BHL-wind boundary conditions (wind along +x).
//         inner_x1 (-x face): INJECTION.  Dirichlet on hydro (rho(y),
//             p=rho(y)*t0, uu^x=gamma(y)*v_x(y)), pinned face B = (0, 0, b0), thermal
//             radiation at t0 boosted with the local wind (same formula as IC).
...
void WindBCs(Mesh *pm) {
  ...
  // X1 hydro BC:
  //   inner_x1 = injection: Dirichlet on
  //              (rho(y), p=rho(y)*t0, uu^x=gamma(y)*v_x(y), 0, 0)
  //   outer_x1 = outflow:   pure copy from innermost active cell
  par_for("hydro_x1_bc", DevExeSpace(),0,(nmb-1),0,(n3-1),0,(n2-1),
  KOKKOS_LAMBDA(int m, int k, int j) {
    if (mb_bcs.d_view(m,BoundaryFace::inner_x1) == BoundaryFlag::user) {
      Real x2v = CellCenterX(j-js, indcs.nx2, size.d_view(m).x2min,
                             size.d_view(m).x2max);
      Real uu_loc = WindPrimitiveVelocity(wind, x2v);
      Real rho_loc = WindDensity(wind, x2v);
      Real pgas_loc = rho_loc * wind.t0;
      for (int i=0; i<ng; ++i) {
        int ig = is - i - 1;
        w0_(m,IDN,k,j,ig) = rho_loc;
        w0_(m,IEN,k,j,ig) = pgas_loc / gm1;
        w0_(m,IVX,k,j,ig) = uu_loc;
  ...
  if (is_radiation_enabled) {
    ...
    KOKKOS_LAMBDA(int m, int n, int k, int j) {
      ...
      if (mb_bcs.d_view(m,BoundaryFace::inner_x1) == BoundaryFlag::user) {
        ...
        ComputeMetricAndInverse(x1v, x2v, x3v, coord.is_minkowski, coord.bh_spin,
                                glower, gupper);
        Real uu1 = WindPrimitiveVelocity(wind, x2v), uu2 = 0.0, uu3 = 0.0;
        Real q   = glower[1][1]*uu1*uu1;
        Real uu0 = sqrt(1.0 + q);
      ...
      if (mb_bcs.d_view(m,BoundaryFace::outer_x1) == BoundaryFlag::user) {
        ...
        Real i_amb = WindEquilibIntensity(m, k, j, ie, nh0,nh1,nh2,nh3, x1v,x2v,x3v,
                          uu_loc, urad_inj, coord.is_minkowski, coord.bh_spin,
