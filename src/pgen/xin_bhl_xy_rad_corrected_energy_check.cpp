...
//========================================================================================
//! \file xin_bhl_xy_rad_corrected.cpp
//! \brief Problem generator: Bondi-Hoyle-Lyttleton wind onto a Kerr BH.
//!        Uniform medium streams past the BH along +x at physical speed v_inf
//!        (input as 'v_wind' in units of c).  Injection on the -x face
//!        (inner_x1); other 5 faces outflow.  Asymptotic B-field is uniform
//!        along +y (perpendicular to both the wind and the spin axis +z),
//!        i.e. theta_B = 90 deg in the convention of Kim & Most 2025
//!        (Phys. Rev. D 111, 083025), Eq. 21.
//!
//! Inputs (all physical, units of c=1, code rho_unit):
//!    rho0        : asymptotic rest-mass density rho_inf
//!    v_wind      : asymptotic wind speed v_inf/c (NOT uu^x; see below)
//!    t0          : pressure-per-density p/rho
//!    beta_target : asymptotic plasma-beta beta_inf = 2 p_gas / b^2  (comoving)
//!
//! Initial state (Cartesian Kerr-Schild):
//!    rho      = rho0                                       outside excision
//!    p        = rho0 * t0                                  (uniform T = t0)
//!    uu^i     = (gamma_inf * v_inf, 0, 0)                  outside excision
//!               (the relativistic normal-frame velocity; reduces to v_inf
//!                in the non-rel limit but includes the gamma factor)
...
//!
//! Boundaries:
//!    inner_x1 (-x face): INJECTION.  Dirichlet on hydro (rho0, p0, uu^x=gamma_inf*v_inf),
//!                       face-centered B pinned to (0, b0, 0), thermal radiation
...
//!
//! PASSIVE ENERGY CHECK: when <problem>/energy_diagnostics=true, detached diagnostic
...
//             uu^x=gamma_inf*v_inf), pinned face B = (0, b0, 0), thermal radiation at t0
...
void WindBCs(Mesh *pm) {
  ...
  // X1 hydro BC:
  //   inner_x1 = injection: Dirichlet on (rho0, p0, uu^x=gamma_inf*v_inf, uu^y=uu^z=0)
...
void WindFluxes(HistoryData *pdata, Mesh *pm) {
  ...
  const bool energy_diag = (pmbp->penergy_diag != nullptr);
  const int rad_hist0 = nradii*nflux;
  const int volume_hist0 = rad_hist0 + (energy_diag ? nradii : 0);

  pdata->nhist = nradii*nflux + (energy_diag ? nradii+5 : 0);
  ...
  for (int g=0; g<nradii; ++g) {
    ...
    if (energy_diag) pdata->label[rad_hist0+g] = "lrad_" + rad_str;
  }
  if (energy_diag) {
    pdata->label[volume_hist0+0] = "qrad_gas";
    pdata->label[volume_hist0+1] = "qrad_abs";
    pdata->label[volume_hist0+2] = "qrad_compt";
    pdata->label[volume_hist0+3] = "qc2p";
    pdata->label[volume_hist0+4] = "qclosure";
  }

  DualArray2D<Real> interpolated_bcc, interpolated_rad_flux;
  for (int g=0; g<nradii; ++g) {
    pdata->hdata[nflux*g+0] = 0.0;
    pdata->hdata[nflux*g+1] = 0.0;
    pdata->hdata[nflux*g+2] = 0.0;
    if (is_mhd) pdata->hdata[nflux*g+3] = 0.0;
    if (energy_diag) pdata->hdata[rad_hist0+g] = 0.0;

    if (energy_diag) {
      grids[g]->InterpolateToSphere(3, pmbp->penergy_diag->rad_energy_flux);
      Kokkos::realloc(interpolated_rad_flux, grids[g]->nangles, 3);
      Kokkos::deep_copy(interpolated_rad_flux, grids[g]->interp_vals);
      interpolated_rad_flux.template modify<DevExeSpace>();
      interpolated_rad_flux.template sync<HostMemSpace>();
    }
    ...
    for (int n=0; n<grids[g]->nangles; ++n) {
      ...
      if (energy_diag) {
        const Real rr_t = drdx*interpolated_rad_flux.h_view(n,0)
                        + drdy*interpolated_rad_flux.h_view(n,1)
                        + drdz*interpolated_rad_flux.h_view(n,2);
        pdata->hdata[rad_hist0+g] += -rr_t*sqrtmdet*domega;
      }
    }
  }

  // Volume-integrated rates from the last completed timestep.  The output/history layer
  // later performs the MPI reduction; this kernel only sums the active leaf cells on-rank.
  if (energy_diag) {
    auto diag = pmbp->penergy_diag->output;
    auto &size = pmbp->pmb->mb_size;
    ...
    Kokkos::parallel_reduce("energy_diag_history",Kokkos::RangePolicy<>(DevExeSpace(),0,nmkji),
    KOKKOS_LAMBDA(const int idx, array_sum::GlobalSum &acc) {
      const int m=idx/nkji;
      const int rem=idx-m*nkji;
      ...
      cell.the_array[0]=diag(m,diagnostics::ED_GAS_RAD_TOTAL,k,j,i)*vol;
      cell.the_array[1]=diag(m,diagnostics::ED_GAS_RAD_ABS,k,j,i)*vol;
      cell.the_array[2]=diag(m,diagnostics::ED_GAS_RAD_COMPT,k,j,i)*vol;
      cell.the_array[3]=diag(m,diagnostics::ED_GAS_C2P,k,j,i)*vol;
      cell.the_array[4]=diag(m,diagnostics::ED_GAS_CLOSURE,k,j,i)*vol;
      acc += cell;
    },Kokkos::Sum<array_sum::GlobalSum>(sum));
    Kokkos::fence();
    for (int n=0; n<5; ++n) pdata->hdata[volume_hist0+n]=sum.the_array[n];
  }
