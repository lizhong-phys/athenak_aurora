...
//========================================================================================
//! \file xin_bhl_xy_rad_corrected_wind_acc_beta_tot_rho_grad.cpp
...
//!        where v_ref=(v_high+v_low)/2, delta_v=(v_high-v_low)/2,
//!        rho_ref=(rho_high+rho_low)/2, and
//!        delta_rho=(rho_high-rho_low)/2.
//!        Injection is on the -x face (inner_x1); the other 5 faces are outflow.
//!        The asymptotic B-field is spatially uniform along +y (perpendicular to
//!        both the reference wind and the spin axis +z), i.e. theta_B = 90 deg
...
//!    v_low       : physical wind speed approached as y -> +infinity
//!    v_high      : physical wind speed approached as y -> -infinity
//!    v_grad_r    : transverse scale of the tanh velocity gradient
//!    rho_low     : rest-mass density approached as y -> +infinity
//!    rho_high    : rest-mass density approached as y -> -infinity
...
//!    A_x      = b0 * x3,  A_y = A_z = 0                    (-> uniform B = b0 y_hat)
//!
//! B-field amplitude b0 (coordinate-frame B^y) is set from the TOTAL thermal
...
//!                       (0, b0, 0), and thermal radiation at t0 boosted with
...
//!
//! Adapted from xin_bhl_x.cpp (B along +z, theta_B = 0 deg).
...
struct bhl_pgen {
  ...
  Real v_low;           // physical v_x approached as y -> +infinity
  Real v_high;          // physical v_x approached as y -> -infinity
  Real v_ref;           // reference physical speed, (v_low + v_high)/2
  Real v_grad_r;        // transverse tanh-gradient scale
  Real rho_low;         // density approached as y -> +infinity
  Real rho_high;        // density approached as y -> -infinity
  Real rho_ref;         // reference density, (rho_low + rho_high)/2
  ...
  Real b0;              // coordinate-frame horizontal field amplitude
};
...
//!        uniform B along +y.
//! Compile with
//! '-D PROBLEM=xin_bhl_xy_rad_corrected_wind_acc_beta_tot_rho_grad'.
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  // primitive velocity.  v_ref is derived rather than read to prevent inconsistent
  // low/high/reference inputs.
  bhl.t0      = pin->GetReal("problem", "t0");
  bhl.v_low   = pin->GetReal("problem", "v_low");
  bhl.v_high  = pin->GetReal("problem", "v_high");
  bhl.v_grad_r = pin->GetReal("problem", "v_grad_r");
  bhl.rho_low = pin->GetReal("problem", "rho_low");
  bhl.rho_high = pin->GetReal("problem", "rho_high");
  bhl.rho_grad_r = pin->GetReal("problem", "rho_grad_r");
  if (bhl.v_low <= 0.0 || bhl.v_low >= 1.0) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
              << "v_low must be in (0, 1); got " << bhl.v_low << std::endl;
    exit(EXIT_FAILURE);
  }
  if (bhl.v_high <= 0.0 || bhl.v_high >= 1.0) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
              << "v_high must be in (0, 1); got " << bhl.v_high << std::endl;
    exit(EXIT_FAILURE);
  }
  if (bhl.v_low > bhl.v_high) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
              << "v_low must not exceed v_high; got v_low=" << bhl.v_low
              << ", v_high=" << bhl.v_high << std::endl;
    exit(EXIT_FAILURE);
  }
  if (bhl.v_grad_r <= 0.0) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
  ...
  if (bhl.rho_low <= 0.0) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl << "rho_low must be positive; got " << bhl.rho_low
              << std::endl;
    exit(EXIT_FAILURE);
  }
  if (bhl.rho_high <= 0.0) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl << "rho_high must be positive; got " << bhl.rho_high
              << std::endl;
    exit(EXIT_FAILURE);
  }
  if (bhl.rho_low > bhl.rho_high) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl << "rho_low must not exceed rho_high; got rho_low="
              << bhl.rho_low << ", rho_high=" << bhl.rho_high << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  bhl.v_ref = 0.5 * (bhl.v_low + bhl.v_high);
  bhl.rho_ref = 0.5 * (bhl.rho_low + bhl.rho_high);

  // excision parameters
  bhl.dexcise = coord.dexcise;
  bhl.pexcise = coord.pexcise;

  // Uniform horizontal field amplitude b0 (coordinate-frame B^y) from the
  ...
  //   uniform B_y via vector potential (below)
...
KOKKOS_INLINE_FUNCTION
Real WindVelocity(const bhl_pgen &pgen, Real y) {
  Real delta_v = 0.5 * (pgen.v_high - pgen.v_low);
...
KOKKOS_INLINE_FUNCTION
Real WindDensity(const bhl_pgen &pgen, Real y) {
  Real delta_rho = 0.5 * (pgen.rho_high - pgen.rho_low);
...
//             p=rho(y)*t0, uu^x=gamma(y)*v_x(y)), pinned face B = (0, b0, 0), thermal
