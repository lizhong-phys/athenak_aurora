...
//========================================================================================
//! \file xin_bhl_xy_rad_corrected_wind_acc_beta_tot.cpp
//! \brief Problem generator: Bondi-Hoyle-Lyttleton wind with a transverse
//!        velocity gradient onto a Kerr BH.  The medium streams along +x with
//!        uniform density and temperature but y-dependent physical speed
//!          v_x(y) = v_ref - delta_v*tanh(y/v_grad_r),
//!        where v_ref=(v_high+v_low)/2 and delta_v=(v_high-v_low)/2.
...
//!
//! Inputs (all physical, units of c=1, code rho_unit):
//!    rho0        : asymptotic rest-mass density rho_inf
...
//!                  = 2*(p_gas+p_rad)/b^2_comoving, evaluated at v_ref
//!
//! Initial state (Cartesian Kerr-Schild):
//!    rho      = rho0                                       outside excision
//!    p        = rho0 * t0                                  (uniform T = t0)
...
//!    b0 = sqrt(2*(p_gas+p_rad)/(beta_target*(1-v_ref^2))).
...
//! varies with y, beta_target is realized exactly at the reference slice y=0;
//! the local comoving beta varies slightly across the velocity gradient.
//!
//! Boundaries:
//!    inner_x1 (-x face): INJECTION.  Dirichlet on hydro
//!                       (rho0, p0, uu^x=gamma(y)*v_x(y)), face-centered B pinned
//!                       to (0, b0, 0), and thermal radiation at t0 boosted with
...
struct bhl_pgen {
  ...
  // Upstream-wind state: uniform rho and p, with a transverse velocity gradient.
...
//! \brief Gradient BHL wind onto a Kerr BH along +x with uniform B along +y.
//! Compile with '-D PROBLEM=xin_bhl_xy_rad_corrected_wind_acc_beta_tot'.
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (bhl.v_low >= bhl.v_high) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
              << "v_low must be less than v_high; got v_low=" << bhl.v_low
  ...
  //   rho and p uniform; uu^x follows the transverse profile outside excision
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    Real uu1, uu2, uu3;
    if (r_excise > 1.0) {
      // Wind state along +x with the local transverse velocity profile.
      rho  = trs.rho0;
      pgas = rho * trs.t0;
...
//             uu^x=gamma(y)*v_x(y)), pinned face B = (0, b0, 0), thermal
...
void WindBCs(Mesh *pm) {
  ...
  // X1 hydro BC:
  //   inner_x1 = injection: Dirichlet on (rho0, p0, uu^x=gamma(y)*v_x(y), 0, 0)
