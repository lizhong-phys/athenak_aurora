...
//========================================================================================
//! \file xin_bondi_final.cpp
//! \brief Bounded Bondi-equilibrium IC around a Kerr BH for gamma = 5/3, with a
//!        single uniform temperature t0 (collapsed t0_low = t0_high = t0).
//!
//! Profiles (anchored at r0 = 1 r_g):
//!   rho(r) = clip( rho_high * (1/r)^(3/2),       rho_low,  rho_high )
//!   T(r)   = t0 = const   (uniform temperature, single parameter)
...
struct bondi_pgen {
  ...
  Real t0;         // uniform temperature; T(r) = t0 everywhere
...
// Uniform temperature everywhere: T(r) = t0.  (aphi_F assumes this constant-T
// case; see its header.  A non-uniform T ~ 1/r would need the old r^(-2) field
// form.)
KOKKOS_INLINE_FUNCTION
...
//
// Uniform-T case (T = t0 const): p_gas(r) = rho(r)*t0 ~ r^(-3/2) in the
...
static Real aphi_F(struct bondi_pgen pgen, Real r) {
  const Real b_high  = sqrt(2.0 * pgen.rho_high * pgen.t0 / pgen.beta_low);
  const Real b_asymp = sqrt(2.0 * pgen.rho_low  * pgen.t0 / pgen.beta_high);
...
//! Compile with '-D PROBLEM=xin_bondi_final' to enroll.
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (bondi.rho_high < bondi.rho_low || bondi.beta_high < bondi.beta_low) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl
              << "Bounded-profile parameters must satisfy: rho_high >= rho_low,"
              << " beta_high >= beta_low" << std::endl;
    exit(EXIT_FAILURE);
  }
