...
//!        Uniform medium streams past the BH at v_wind ~ 0.1 c along +z.
//!        Represents a BH that physically plunges through an AGN-disk layer,
//!        transformed to the BH rest frame.  In that frame the disk gas
//!        appears as a uniform supersonic wind incident on the -z face.
//!
//! Initial state (Cartesian Kerr-Schild):
//!    rho      = rho0                                       outside excision
//!    p        = rho0 * t0                                  (isothermal)
//!    uu^i     = (0, 0, v_wind)                             outside excision
...
//!
//! Boundaries:
//!    inner_x3 (-z face): INJECTION.  Dirichlet on hydro (rho0, p0, uu^z=v_wind),
...
//!
//! Adapted from xin.cpp (Gaussian-dip Bondi reservoir).
...
// Parameters for the BHL-wind setup
struct bhl_pgen {
  ...
  //   rho = rho0,  p = rho0*t0,  uu^i = (0, 0, v_wind)
  Real rho0;            // uniform background rest-mass density
  Real t0;              // isothermal temperature
  Real v_wind;          // wind speed along +z (units of c; valid for v_wind << 1)
...
//! \brief BHL wind onto a Kerr BH, optionally threaded by a uniform vertical
//!        B-field. See file header for full description.
//! Compile with '-D PROBLEM=xin_bhl' to enroll as user-specific problem generator.
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (!pmbp->pcoord->is_general_relativistic &&
    ...
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
              << "xin_bhl problem can only be run when GR defined in <coord> block"
              << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  //   uniform IC: rho = rho0, p = rho0*t0, uu^z = v_wind
  ...
  //   uniform: rho = rho0, p = rho0*t0, uu^i = (0, 0, v_wind) outside excision
  ...
  if (pmbp->pmhd != nullptr) {
    ...
    // No global beta-renormalization needed: bhl.b0 was set analytically from
...
//----------------------------------------------------------------------------------------
//! \fn WindBCs
//  \brief BHL-wind boundary conditions.
//         inner_x3 (-z face): INJECTION.  Dirichlet on hydro (rho0, p0,
//             uu^z=v_wind), pinned face B = (0, 0, b0), thermal radiation at t0
...
void WindBCs(Mesh *pm) {
  ...
  // X1-Boundary fields: zero-gradient outflow on both faces
  if (pm->pmb_pack->pmhd != nullptr) {
    auto &b0 = pm->pmb_pack->pmhd->b0;
    par_for("outflow_field_x1", DevExeSpace(),0,(nmb-1),0,(n3-1),0,(n2-1),
  ...
  // Zero-gradient (pure copy) outflow on X1 ghost zones
  par_for("outflow_hydro_x1", DevExeSpace(),0,(nmb-1),0,(n3-1),0,(n2-1),
  KOKKOS_LAMBDA(int m, int k, int j) {
    if (mb_bcs.d_view(m,BoundaryFace::inner_x1) == BoundaryFlag::user) {
      for (int i=0; i<ng; ++i) {
        int ig = is - i - 1;
        w0_(m,IDN,k,j,ig) = w0_(m,IDN,k,j,is);
        w0_(m,IEN,k,j,ig) = w0_(m,IEN,k,j,is);
  ...
  // X1 radiation BC: no incoming intensity on either face (outflow).
  if (is_radiation_enabled) {
  ...
  // X3-Boundary fields:
  //   inner_x3 = injection: pin uniform vertical B  (x1f=0, x2f=0, x3f=bz_inj)
  //   outer_x3 = outflow:   copy interior face values
  if (pm->pmb_pack->pmhd != nullptr) {
    auto &b0 = pm->pmb_pack->pmhd->b0;
    par_for("bfield_x3_bc", DevExeSpace(),0,(nmb-1),0,(n2-1),0,(n1-1),
    KOKKOS_LAMBDA(int m, int j, int i) {
      if (mb_bcs.d_view(m,BoundaryFace::inner_x3) == BoundaryFlag::user) {
        for (int k=0; k<ng; ++k) {
          b0.x1f(m,ks-k-1,j,i) = 0.0;
          if (i == n1-1) {b0.x1f(m,ks-k-1,j,i+1) = 0.0;}
          b0.x2f(m,ks-k-1,j,i) = 0.0;
          if (j == n2-1) {b0.x2f(m,ks-k-1,j+1,i) = 0.0;}
          b0.x3f(m,ks-k-1,j,i) = bz_inj;
        }
      }
  ...
  // X3 hydro BC:
  //   inner_x3 = injection: Dirichlet on (rho0, p0, uu^x=uu^y=0, uu^z=v_wind)
  //   outer_x3 = outflow:   pure copy from innermost active cell
  par_for("hydro_x3_bc", DevExeSpace(),0,(nmb-1),0,(n2-1),0,(n1-1),
  KOKKOS_LAMBDA(int m, int j, int i) {
    if (mb_bcs.d_view(m,BoundaryFace::inner_x3) == BoundaryFlag::user) {
      for (int k=0; k<ng; ++k) {
        int kg = ks - k - 1;
        w0_(m,IDN,kg,j,i) = rho_inj;
        w0_(m,IEN,kg,j,i) = pgas_inj / gm1;
        w0_(m,IVX,kg,j,i) = 0.0;
        w0_(m,IVY,kg,j,i) = 0.0;
        w0_(m,IVZ,kg,j,i) = v_wind;
      }
    }
    if (mb_bcs.d_view(m,BoundaryFace::outer_x3) == BoundaryFlag::user) {
      for (int k=0; k<ng; ++k) {
  ...
  if (is_radiation_enabled) {
    // inner_x3 = injection: thermal radiation at t0 boosted with the wind
    //                       (same formula as IC, evaluated at active cell ks).
    // outer_x3 = outflow:   no incoming intensity, copy outgoing.
    par_for("rad_x3_bc", DevExeSpace(),0,(nmb-1),0,nang1,0,(n2-1),0,(n1-1),
    KOKKOS_LAMBDA(int m, int n, int j, int i) {
      ...
      Real nh3 = nh_c_.d_view(n,3);
      if (mb_bcs.d_view(m,BoundaryFace::inner_x3) == BoundaryFlag::user) {
        // Metric at active cell (ks, j, i) for q = g_{ij} uu^i uu^j
        Real &x1min = size.d_view(m).x1min;
        Real &x1max = size.d_view(m).x1max;
        Real &x2min = size.d_view(m).x2min;
        Real &x2max = size.d_view(m).x2max;
        ...
        Real x1v = CellCenterX(i-is, indcs.nx1, x1min, x1max);
        Real x2v = CellCenterX(j-js, indcs.nx2, x2min, x2max);
        Real x3v = CellCenterX(0,    indcs.nx3, x3min, x3max);  // ks → local 0
        Real glower[4][4], gupper[4][4];
        ComputeMetricAndInverse(x1v, x2v, x3v, coord.is_minkowski, coord.bh_spin,
                                glower, gupper);
        Real uu1 = 0.0, uu2 = 0.0, uu3 = v_wind;
        Real q   = glower[3][3]*uu3*uu3;
        Real uu0 = sqrt(1.0 + q);
        Real ut0 = (norm_to_tet_(m,0,0,ks,j,i)*uu0 + norm_to_tet_(m,0,3,ks,j,i)*uu3);
        Real ut1 = (norm_to_tet_(m,1,0,ks,j,i)*uu0 + norm_to_tet_(m,1,3,ks,j,i)*uu3);
        Real ut2 = (norm_to_tet_(m,2,0,ks,j,i)*uu0 + norm_to_tet_(m,2,3,ks,j,i)*uu3);
        Real ut3 = (norm_to_tet_(m,3,0,ks,j,i)*uu0 + norm_to_tet_(m,3,3,ks,j,i)*uu3);
        Real un_t = ut1*nh1 + ut2*nh2 + ut3*nh3;
        Real n0_f = ut0*nh0 - un_t;
        Real n0 = tc(m,0,0,ks,j,i);
        Real n_0 = tetcov_c_(m,0,0,ks,j,i)*nh0 + tetcov_c_(m,1,0,ks,j,i)*nh1 +
                   tetcov_c_(m,2,0,ks,j,i)*nh2 + tetcov_c_(m,3,0,ks,j,i)*nh3;
        Real i_inj = n0*n_0*(urad_inj/(4.0*M_PI))/SQR(SQR(n0_f));
        for (int k=0; k<ng; ++k) {
          i0_(m,n,ks-k-1,j,i) = i_inj;
        }
      }
      if (mb_bcs.d_view(m,BoundaryFace::outer_x3) == BoundaryFlag::user) {
