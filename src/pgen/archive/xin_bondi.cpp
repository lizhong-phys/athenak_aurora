...
//========================================================================================
//! \file xin_bondi.cpp
//! \brief Bounded Bondi-equilibrium IC around a Kerr BH for gamma = 5/3.
//!
//! Profiles (anchored at r0 = 1 r_g):
//!   rho(r) = clip( rho_high * (1/r)^(3/2),       rho_low,  rho_high )
//!   T(r)   = T_ini = const   (uniform temperature; set t0_low = t0_high)
//!   beta(r)= clip( beta_low * r^(3/2),           beta_low, beta_high)
//!
//! Derived locally:
//!   p_gas(r) = rho(r) * T(r)                          (gas EOS: P = rho T in code)
//!   u_rad(r) = a_rad * T(r)^4                          (radiation in thermal eq.)
//!   p_mag(r) = p_gas(r) / beta(r)                      (gas-only beta definition)
//!   b(r)     = sqrt( 2 * p_gas(r) / beta(r) )
//!
//! With uniform T, p_gas(r) ~ r^(-3/2) and beta(r) ~ r^(3/2), so the equatorial
//! target field is b(r) ~ r^(-3/2) in the unclipped region.  (The earlier r^(-2)
//! form assumed the adiabatic T ~ r^(-1) profile; it is no longer consistent with
//! beta ~ r^(3/2) once T is constant.)
//!
//! Magnetic field via A_phi(r,theta) = F(r) * sin^2(theta), where
//!   F(r) = integral_0^r R' * b(R') dR'
//! Construction guarantees B_z(R, z=0) = b(R) exactly on the equator, so the
//! local target magnetization (beta(r)=p_gas/p_mag) is honored at every radius.
//! For the uniform-T power-law profile, F(r) is piecewise analytic:
//!   r <= 1          : F = 0.5 * b_high * r^2           (b clipped to b_high)
//!   1 < r <= r_out  : F = b_high * (2*sqrt(r) - 1.5)   (b proportional to r^(-3/2))
//!   r > r_out       : F = 0.5 * b_asymp * (r^2 - r_out^2) + F(r_out)
//! where r_out = (rho_high/rho_low)^(2/3); choose beta_high/beta_low =
//! rho_high/rho_low so the rho and beta clips coincide at the same r_out.
//! curl(A) via CT gives face-centered B that is exactly div-free.
//!
...
struct bondi_pgen {
  ...
  // Bounded Bondi-equilibrium parameters (anchored at r0 = 1 r_g)
  Real rho_high;   // density cap (near BH)
  Real rho_low;    // density floor (asymptotic)
  Real t0_high;    // temperature cap (near BH); T = t0 * T_unit in physical
  Real t0_low;     // temperature floor (asymptotic)
  Real beta_low;   // beta lower bound (near BH, most magnetized)
  Real beta_high;  // beta upper bound (asymptotic, least magnetized)
};
...
// Bondi-equilibrium profile helpers (γ = 5/3, anchored at r₀ = 1)
KOKKOS_INLINE_FUNCTION
static Real rho_profile(struct bondi_pgen pgen, Real r) {
  if (r < 1.0) return pgen.rho_high;
  Real rho = pgen.rho_high * pow(r, -1.5);
  return fmax(pgen.rho_low, fmin(pgen.rho_high, rho));
}

// Uniform temperature: set t0_low = t0_high in the input, which makes the clip
// below return T = t0_high for all r >= 1.  (aphi_F assumes this constant-T case;
// see its header.  A non-uniform T ~ 1/r would need the old r^(-2) field form.)
KOKKOS_INLINE_FUNCTION
static Real T_profile(struct bondi_pgen pgen, Real r) {
  if (r < 1.0) return pgen.t0_high;
  Real T = pgen.t0_high / r;
  return fmax(pgen.t0_low, fmin(pgen.t0_high, T));
}

KOKKOS_INLINE_FUNCTION
static Real beta_profile(struct bondi_pgen pgen, Real r) {
  if (r < 1.0) return pgen.beta_low;
  Real beta = pgen.beta_low * pow(r, 1.5);
  return fmin(pgen.beta_high, fmax(pgen.beta_low, beta));
}

KOKKOS_INLINE_FUNCTION
static Real b_profile(struct bondi_pgen pgen, Real r) {
  Real rho_l  = rho_profile(pgen, r);
  Real T_l    = T_profile(pgen, r);
  Real beta_l = beta_profile(pgen, r);
  Real pgas_l = rho_l * T_l;
  return sqrt(2.0 * pgas_l / beta_l);
}

// F(r) = integral_0^r R' * b(R') dR'.
// Used in A_phi = F(r) * sin^2(theta), which gives B_z(R, z=0) = b(R) on the
// equator, so the local target magnetization beta(r) = p_gas/p_mag is honored.
//
// Uniform-T case (t0_low = t0_high): p_gas(r) = rho(r)*T_ini ~ r^(-3/2) in the
// unclipped region, and beta(r) ~ r^(3/2), so the equatorial target field is
//   b(r) = sqrt(2 p_gas/beta) = b_high * r^(-3/2)   (NOT r^(-2): that older form
//   assumed the adiabatic T ~ r^(-1) profile).  Re-integrating R'*b(R'):
//   r <= 1          : F = 0.5 * b_high * r^2
//   1 < r <= r_out  : F = b_high * (2*sqrt(r) - 1.5)
//   r > r_out       : F = 0.5 * b_asymp * (r^2 - r_out^2) + F(r_out)
// F is continuous at r=1 (= 0.5*b_high from both sides) and at r_out.
// Assumes rho and beta saturate at the same r_out = (rho_high/rho_low)^(2/3),
// i.e. choose beta_high/beta_low = rho_high/rho_low.
KOKKOS_INLINE_FUNCTION
static Real aphi_F(struct bondi_pgen pgen, Real r) {
  const Real b_high  = sqrt(2.0 * pgen.rho_high * pgen.t0_high / pgen.beta_low);
  const Real b_asymp = sqrt(2.0 * pgen.rho_low  * pgen.t0_low  / pgen.beta_high);
  const Real r_out   = pow(pgen.rho_high / pgen.rho_low, 2.0/3.0);
  const Real F_at_out = b_high * (2.0*sqrt(r_out) - 1.5);

  if (r <= 1.0) {
    return 0.5 * b_high * r * r;
  } else if (r <= r_out) {
    return b_high * (2.0*sqrt(r) - 1.5);
  } else {
    return 0.5 * b_asymp * (r*r - r_out*r_out) + F_at_out;
  }
}
...
//! \brief Bounded Bondi-equilibrium IC around a Kerr BH for gamma = 5/3.
//! Compile with '-D PROBLEM=xin_bondi' to enroll.
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  // Read bounded-profile parameters
  bondi.rho_high  = pin->GetReal("problem", "rho_high");
  bondi.rho_low   = pin->GetReal("problem", "rho_low");
  bondi.t0_high   = pin->GetReal("problem", "t0_high");
  bondi.t0_low    = pin->GetReal("problem", "t0_low");
  bondi.beta_low  = pin->GetReal("problem", "beta_low");
  bondi.beta_high = pin->GetReal("problem", "beta_high");
  ...
  // Equilibrium IC: set v_r = -sqrt(p_tot/rho) (inward, p_ram = p_tot at each cell)
  // instead of v = 0.  Removes the impulsive v=0 -> free-fall transient.
  bool equilibrium_ic = pin->GetOrAddBoolean("problem", "equilibrium_ic", false);

  // Sanity check: bounds must be ordered
  if (bondi.rho_high < bondi.rho_low || bondi.t0_high < bondi.t0_low ||
      bondi.beta_high < bondi.beta_low) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl
              << "Bounded-profile parameters must satisfy: rho_high >= rho_low,"
              << " t0_high >= t0_low, beta_high >= beta_low" << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    // Initial velocity.  Default v = 0 (gas relaxes under gravity + B).
    // If equilibrium_ic = true, set v_r = -sqrt(p_tot/rho) inward (Cartesian KS
    // approximation), so that ram pressure balances the local total pressure
    // (p_gas + p_mag + p_rad) at every cell.  This eliminates the impulsive
    // v=0 -> free-fall transient.
    Real uu1 = 0.0, uu2 = 0.0, uu3 = 0.0;
    if (equilibrium_ic && r_excise > 1.0) {
      Real beta_l  = beta_profile(trs, r);
      Real pmag_l  = pgas / beta_l;                       // p_gas / beta_local
      Real prad_l  = urad / 3.0;                          // u_rad = a*T^4, p_rad = u_rad/3
      Real p_tot_l = pgas + pmag_l + prad_l;
      Real v_r_mag = sqrt(p_tot_l / fmax(rho, 1.0e-30));
      Real r_cart  = fmax(sqrt(SQR(x1v) + SQR(x2v) + SQR(x3v)), 1.0e-6);
      uu1 = -v_r_mag * x1v / r_cart;
      uu2 = -v_r_mag * x2v / r_cart;
      uu3 = -v_r_mag * x3v / r_cart;
    }
...
//         rho = rho_low, T = t0_low, p = rho_low*t0_low, v copied from interior.
...
void ReservoirBondiBounded(Mesh *pm) {
  ...
  const Real T_res    = bondi.t0_low;
  const Real pgas_res = bondi.rho_low * bondi.t0_low;
