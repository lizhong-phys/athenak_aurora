...
//========================================================================================
//! \file xin_new.cpp
//! \brief Bondi-like IC: rho = rho0 * r^(-3/2), flat for r < r_g, floored at rho_low.
//!        Newtonian free-fall velocity v_r = -sqrt(2M/r). Isothermal p = rho * t0.
//!        Uniform vertical B-field via vector potential.
...
struct bondi_pgen {
  ...
  Real rho0;            // density at r = r_g (code units)
  Real rho_low;         // outer density floor (code units)
  Real t0;              // isothermal temperature p/rho
  Real v_max;           // cap on |v_r| for free-fall init
...
//! \brief Bondi-like IC around a Kerr BH with uniform vertical B-field.
//! Compile with '-D PROBLEM=xin_new' to enroll.
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  bondi.v_max   = pin->GetOrAddReal("problem", "v_max", 0.3);
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    if (r_excise > 1.0) {
      // Bondi power-law: rho = rho0 * r^(-3/2), flat inside r_g, floored at rho_low
      if (r < 1.0) {
        rho = trs.rho0;
      } else {
        rho = trs.rho0 * pow(r, -1.5);
        rho = fmax(rho, trs.rho_low);
      }
      pgas = rho * trs.t0;
    ...
    if (r_excise > 1.0 && r > 1.0) {
      Real r_cart = sqrt(SQR(x1v) + SQR(x2v) + SQR(x3v));
      if (r_cart > 0.0) {
        Real vr = -fmin(sqrt(2.0 / r), trs.v_max);
        uu1 = vr * x1v / r_cart;
        uu2 = vr * x2v / r_cart;
        uu3 = vr * x3v / r_cart;
      }
