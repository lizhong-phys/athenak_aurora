...
//!
//! AthenaK port of the Athena++ problem generator rec_shear.cpp.  Radiation has been
//! removed and the EOS is restricted to isothermal, so the setup reduces to:
//!
...
//!         Bz = bx2by * bz2by * By0         (uniform vertical field)
//!
...
//!
//!   * Optional divergence-free Bz perturbation localized on the two sheets, seeded from
//!     a vector potential A_y so that div(B) = 0 to machine precision on the staggered
//!     mesh (this differs from the Athena++ version, where the analytic Bz perturbation
//!     was not discretely divergence-free and therefore had to be switched off).
//!
//!   * Small random velocity perturbations, with the mean momentum removed.
//!
...
//! carried by the remap and must NOT be added to the initial y-momentum (unlike the
//! Athena++ version, which added it whenever orbital advection was off).
//!
...
namespace {
// Parameters shared between the pgen and the user source/history functions.  These live
// at file scope and are copied into local variables before use inside device kernels.
...
//! Equals +1 at both z-boundaries and -1 between the two sheets.

KOKKOS_INLINE_FUNCTION
Real HarrisProfile(const Real z, const Real zl, const Real zu, const Real jw) {
  return tanh((z - zu)/jw) - tanh((z - zl)/jw) + 1.0;
}

//----------------------------------------------------------------------------------------
//! \fn SheetEnvelope
//! \brief Gaussian envelope peaked on each of the two current sheets, used for the
//! seed perturbation of the vertical field.

KOKKOS_INLINE_FUNCTION
Real SheetEnvelope(const Real z, const Real zl, const Real zu, const Real jw) {
  return exp(-0.5*SQR((z - zu)/jw)) + exp(-0.5*SQR((z - zl)/jw));
}

//----------------------------------------------------------------------------------------
//! \fn VecPotY
//! \brief y-component of the vector potential generating the seed perturbation:
//!     A_y = -(Bz0/kx) cos(kx*x) * [gaussians centred on the two sheets]
//! so that Bz = dA_y/dx = Bz0 sin(kx*x)*envelope and Bx = -dA_y/dz.  Taking the discrete
//! curl of this guarantees div(B) = 0 exactly on the staggered mesh.

KOKKOS_INLINE_FUNCTION
Real VecPotY(const Real x, const Real z, const Real bz0, const Real kx,
             const Real zl, const Real zu, const Real jw) {
  if (bz0 == 0.0) return 0.0;
  return -(bz0/kx)*cos(kx*x)*SheetEnvelope(z, zl, zu, jw);
}
}  // namespace
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  // enroll user history and (if needed) user source terms
  ...
  // no perturbation, reproducing Bz0 = 0 in the Athena++ version.
  ...
  // field strengths from the plasma betas
  ...
  // copy file-scope values into locals for capture by device lambdas
  Real zl = z_lower_, zu = z_upper_;

  // ---- capture mesh variables for kernels --------------------------------------------
  ...
  // Equilibrium part is set analytically: Bx and By depend on z only and Bz is uniform,
  // so div(B) = 0 identically on the staggered mesh.  The seed perturbation is added as
  // the discrete curl of A_y, which is also exactly divergence free.
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    Real bz = by0*bx2by*bz2bx
              + (VecPotY(x1fp, x3f, bz0, kx, zl, zu, jwidth)
               - VecPotY(x1f,  x3f, bz0, kx, zl, zu, jwidth))/dx1;
    ...
    if (i==ie) {
      b0.x1f(m,k,j,ie+1) = by0*bx2by*HarrisProfile(x3v, zl, zu, jwidth)
                           - (VecPotY(x1fp, x3fp, bz0, kx, zl, zu, jwidth)
                            - VecPotY(x1fp, x3f,  bz0, kx, zl, zu, jwidth))/dx3;
    }
    if (j==je) {
      b0.x2f(m,k,je+1,i) = by;
    }
    if (k==ke) {
      b0.x3f(m,ke+1,j,i) = by0*bx2by*bz2bx
                           + (VecPotY(x1fp, x3fp, bz0, kx, zl, zu, jwidth)
                            - VecPotY(x1f,  x3fp, bz0, kx, zl, zu, jwidth))/dx1;
    }
  });
  ...
  Real dcoef = SQR(by0/iso_cs)*(1.0 + SQR(bx2by))/2.0;
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    Real sprof = HarrisProfile(x3v, zl, zu, jwidth);
    Real rd = d0 + dcoef*(1.0 - SQR(sprof));

    // random velocity perturbations to seed reconnection/turbulence
    ...
    u0(m,IDN,k,j,i) = rd;
    // NOTE: no -q*Omega0*x term here; the background shear is handled by AthenaK's
    // orbital advection, which is always active in a 3D shearing box.
  ...
  if (zero_net_mom) {
    ...
    Real m1sum = 0.0, m2sum = 0.0, m3sum = 0.0;
    Kokkos::parallel_reduce("rec_shear_mom", Kokkos::RangePolicy<>(DevExeSpace(),0,nmkji),
    KOKKOS_LAMBDA(const int &idx, Real &s1, Real &s2, Real &s3) {
      int m = (idx)/nkji;
      ...
      s2 += u0(m,IM2,k,j,i);
      s3 += u0(m,IM3,k,j,i);
    }, Kokkos::Sum<Real>(m1sum), Kokkos::Sum<Real>(m2sum), Kokkos::Sum<Real>(m3sum));
    Kokkos::fence();

    Real msum[3] = {m1sum, m2sum, m3sum};
#if MPI_PARALLEL_ENABLED
    ...
    Real mavg1 = msum[0]/ncells;
    Real mavg2 = msum[1]/ncells;
    Real mavg3 = msum[2]/ncells;
...
void RecShearSrcTerms(Mesh *pm, const Real bdt) {
  ...
  // copy file-scope parameters into locals for device capture
  Real grav = grav_;
  Real zmid = z_mid_, zmin = z_min_, zmax = z_max_;
  Real zl = z_lower_, zu = z_upper_;
  Real swidth = sinkwidth_, tausink = tau_sink_, dfloor = dfloor_;

  Real gcoef = grav*bdt*SQR(omega0);
  Real zsink_l = zmid - 0.5*swidth, zsink_u = zmid + 0.5*swidth;
  Real zbot_u  = zmin + 0.5*swidth;
  Real ztop_l  = zmax - 0.5*swidth;
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    if (swidth > 0.0) {
      ...
      if ((x3v >= zsink_l) && (x3v <= zsink_u)) {
        window = SQR(cos(M_PI*(x3v - zmid)/swidth));
      } else if ((x3v >= zmin) && (x3v <= zbot_u)) {
        window = SQR(cos(0.5*M_PI*(x3v - zmin)/swidth));
      } else if ((x3v >= ztop_l) && (x3v <= zmax)) {
        window = SQR(cos(0.5*M_PI*(x3v - zmax)/swidth));
      }
      if (window > 0.0) {
        // exponential decay factor, stable for any bdt/tau_sink ratio
        Real fac = exp(-window*bdt/tausink);
        // do not drain below the density floor
        Real dden = fmax(den*fac, dfloor) - den;
        u0(m,IDN,k,j,i) += dden;
