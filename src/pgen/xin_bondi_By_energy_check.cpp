// Isolated Bondi diagnostic pgen; parent physics copied without modification.
//========================================================================================
// AthenaXXX astrophysical plasma code
// Copyright(C) 2020 James M. Stone <jmstone@ias.edu> and the Athena code team
// Licensed under the 3-clause BSD License (the "LICENSE")
//========================================================================================
//! \file xin_bondi_By.cpp
//! \brief Problem generator: uniform gas at rest around a Kerr black hole, threaded
//!        by an (optional) uniform horizontal (B_y) field. Used to test relaxation toward
...
//! and replacing the density-weighted poloidal field with a uniform B_y.
...
// Parameters for Gaussian-dip IC with uniform horizontal (B_y) field
struct bondi_pgen {
  ...
  // Uniform horizontal magnetic field B = b0 * y_hat:
  // A_x = b0 * x3 (A_y = A_z = 0), giving B = curl A = (0, b0, 0) = b0 y-hat,
  // divergence-free everywhere by construction (B = curl A on faces).
  // Set b0 = 0.0 to disable MHD initialization.
  Real b0;              // horizontal (B^y) field amplitude
};
...
//!        by a uniform horizontal (B_y) field. See file header for full description.
//! Compile with '-D PROBLEM=xin_bondi_By_energy_check'.

void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  MeshBlockPack *pmbp = pmy_mesh_->pmb_pack;
  if (global_variable::my_rank == 0) {
    std::cout << "BONDI_ENERGY_DIAG field=By physics=xin_bondi_By"
              << " enabled=" << (pmbp->penergy_diag != nullptr)
  ...
  // the BC is ever switched to pin a uniform B_y.
  ...
  //   rho(r, t=0) = rho0  (uniform; no dip)
  //   p = rho * t0,  uu^i = 0,  uniform B_y via vector potential (below)
  ...
  if (pmbp->pmhd != nullptr) {
    // Uniform horizontal B_y via target plasma beta; bondi.b0 was already
...
//----------------------------------------------------------------------------------------
// Vector potential for a UNIFORM HORIZONTAL field  B = b0 * y_hat  (coordinate-frame B^y):
//   A_x = b0 * x3,  A_y = A_z = 0   =>   B = curl A = (0, b0, 0).
// Same flat construction as the BHL pgen (replaces the axisymmetric A_phi used for B_z);
// the field is uniform in Cartesian Kerr-Schild coordinates and points along +y.

//----------------------------------------------------------------------------------------
// Function to compute 1-component of vector potential.

KOKKOS_INLINE_FUNCTION
Real A1(struct bondi_pgen pgen, Real x1, Real x2, Real x3) {
  return pgen.b0 * x3;          // A_x = b0 * z  ->  B^y = dA_x/dz = b0
}
...
//         Pins ghost zones to (rho0, p=rho0*t0, v=0, B_y=b0). Radiation: OUTGOING
...
void ReservoirBondi(Mesh *pm) {
  ...
  // X1-Boundary: zero-gradient copy of interior face B (uniform B_y from IC)
  ...
  // X2-Boundary: zero-gradient copy of interior face B (uniform B_y from IC)
  if (pm->pmb_pack->pmhd != nullptr) {
    auto &b0 = pm->pmb_pack->pmhd->b0;
  ...
  // X3-Boundary: zero-gradient copy of interior face B (uniform B_y from IC)
  if (pm->pmb_pack->pmhd != nullptr) {
    auto &b0 = pm->pmb_pack->pmhd->b0;
